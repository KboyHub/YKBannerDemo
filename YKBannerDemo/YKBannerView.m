...
@interface YKBannerView()<UIScrollViewDelegate>
//轮播的图片数组
...
//创建用来缓存图片的文件夹
+ (void)initialize {
    NSString *cache = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"YKBanner"];
    BOOL isDir = NO;
...
+ (instancetype)bannerViewWithImageArray:(NSArray *)imageArray describeArray:(NSArray *)describeArray {
    YKBannerView *bannerView = [[self alloc] init];
...
- (void)downloadImages:(int)index {
    ...
    //从沙盒中取图片
    NSString *path = [[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"YKBanner"] stringByAppendingPathComponent:[key lastPathComponent]];
