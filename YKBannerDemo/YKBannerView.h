...
/**
 *  该方法用来处理图片的点击，会返回图片所在数组中的索引
 *  代理与block二选一即可，若两者都实现，block的优先级高
 *
 *  @param bannerView 控件本身
 *  @param index        图片索引
 */
- (void)carouselView:(YKBannerView *)bannerView clickImageAtIndex:(NSInteger)index;
...
/**
 *  设置图片轮播模式，默认为ChangeModeDefault
 */
@property (nonatomic, assign) ChangeMode changeMode;


/**
 *  设置小圆点位置，默认为PositionBottomCenter
 ...
 *  代理，用来处理图片的点击
 */
@property (nonatomic, weak) id<YKBannerViewDelegate> delegate;
